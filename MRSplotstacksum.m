function MRSplotstacksum(MRS_struct)
...
numspec = length(MRS_struct.sumspec(:,1));

% Find Water amplitude max, across all Pfiles
waterheight = abs(max(MRS_struct.waterspec,[],2));
heightrescale = repmat((1./waterheight), [1 length(MRS_struct.sumspec(1,:))]);
SpectraToPlot = MRS_struct.sumspec .* heightrescale;
...
gabaheight = real(max(SpectraToPlot(:,17250:18000),[],2));
...
SpectraToPlot = SpectraToPlot + ...
    repmat(plotstackoffset, [ 1  length(MRS_struct.sumspec(1,:))]);
